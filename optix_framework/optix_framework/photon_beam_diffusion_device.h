...
_fn float pbd_bssrdf(optix::float3 xi, optix::float3 ni, optix::float3 wt, optix::float3 xo, optix::float3 no, optix::float4 props, optix::float4 C) {
	...
	float w_exp = optix::clamp((length(x) - a) / (b - a), 0.0, 1.0);
	...
	for (int i = 1.0; i <= N; ++i) {
		...
		float theta_j = optix::lerp(theta_a, theta_b, xi_i);
	...
	return optix::fmaxf(S_d, 0.0) / N;
}

_fn optix::float3 photon_beam_diffusion_bssrdf(const BSSRDFGeometry & geometry, const float recip_ior, const MaterialDataCommon& material, unsigned int flags, TEASampler & sampler)
{
