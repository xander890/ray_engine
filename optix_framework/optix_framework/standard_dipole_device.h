	...
	const MaterialDataCommon& material, unsigned int flags, TEASampler & sampler)
{
	...
	float3 r_sqr = optix::make_float3(dist*dist);

	// Compute distances to dipole sources
	float3 d_r_sqr = r_sqr + real_source;
	float3 d_r = optix::make_float3(sqrtf(d_r_sqr.x), sqrtf(d_r_sqr.y), sqrtf(d_r_sqr.z));
	float3 d_v_sqr = r_sqr + virtual_source;
	float3 d_v = optix::make_float3(sqrtf(d_v_sqr.x), sqrtf(d_v_sqr.y), sqrtf(d_v_sqr.z));
