...
//-----------------------------------------------------------------------------
// 
// GLUTDisplayTest class implementation 
//-----------------------------------------------------------------------------

Mouse*         GLUTDisplayWrapper::m_mouse                = 0;
PinholeCamera* GLUTDisplayWrapper::m_camera               = 0;
SampleScene*   GLUTDisplayWrapper::m_scene                = 0;

double         GLUTDisplayWrapper::m_last_frame_time      = 0.0;
unsigned int   GLUTDisplayWrapper::m_last_frame_count     = 0;
unsigned int   GLUTDisplayWrapper::m_frame_count          = 0;

bool           GLUTDisplayWrapper::m_display_fps          = true;
double         GLUTDisplayWrapper::m_fps_update_threshold = 0.5;
char           GLUTDisplayWrapper::m_fps_text[32];
float3         GLUTDisplayWrapper::m_text_color           = make_float3( 0.95f );
float3         GLUTDisplayWrapper::m_text_shadow_color    = make_float3( 0.10f );

bool           GLUTDisplayWrapper::m_print_mem_usage      = false;

GLUTDisplayWrapper::contDraw_E GLUTDisplayWrapper::m_app_continuous_mode = CDNone;
GLUTDisplayWrapper::contDraw_E GLUTDisplayWrapper::m_cur_continuous_mode = CDNone;

bool           GLUTDisplayWrapper::m_display_frames       = true;
bool           GLUTDisplayWrapper::m_save_frames_to_file  = false;
std::string    GLUTDisplayWrapper::m_save_frames_basename = "";

std::string    GLUTDisplayWrapper::m_camera_pose          = "";

int            GLUTDisplayWrapper::m_initial_window_width  = -1;
int            GLUTDisplayWrapper::m_initial_window_height = -1;

int            GLUTDisplayWrapper::m_old_window_height    = -1;
int            GLUTDisplayWrapper::m_old_window_width     = -1;
int            GLUTDisplayWrapper::m_old_window_x         = -1;
int            GLUTDisplayWrapper::m_old_window_y         = -1;
int            GLUTDisplayWrapper::m_old_window_x_offset  = -1;
int            GLUTDisplayWrapper::m_old_window_y_offset  = -1;

unsigned int   GLUTDisplayWrapper::m_texId                = 0;
bool           GLUTDisplayWrapper::m_sRGB_supported       = false;
bool           GLUTDisplayWrapper::m_use_sRGB             = false;

bool           GLUTDisplayWrapper::m_initialized          = false;
bool           GLUTDisplayWrapper::m_requires_display     = false;
bool           GLUTDisplayWrapper::m_benchmark_no_display = false;
unsigned int   GLUTDisplayWrapper::m_warmup_frames        = 50u;
unsigned int   GLUTDisplayWrapper::m_timed_frames         = 100u;
double         GLUTDisplayWrapper::m_warmup_start         = 0;
double         GLUTDisplayWrapper::m_warmup_time          = 10.0;
double         GLUTDisplayWrapper::m_benchmark_time       = 10.0;
unsigned int   GLUTDisplayWrapper::m_benchmark_frame_start= 0;
double         GLUTDisplayWrapper::m_benchmark_frame_time = 0;
std::string    GLUTDisplayWrapper::m_title                = "";

double         GLUTDisplayWrapper::m_progressive_timeout   = -1.;
double         GLUTDisplayWrapper::m_start_time           = 0.0;

int            GLUTDisplayWrapper::m_num_devices          = 0;

bool           GLUTDisplayWrapper::m_enable_cpu_rendering = false;
...
void GLUTDisplayWrapper::printUsage()
{
  std::cerr
    << "Standard options:\n"
  ...
  if ( !GLUTDisplayWrapper::m_requires_display )
  {
    std::cerr
...
void GLUTDisplayWrapper::init( int& argc, char** argv )
{
  m_initialized = true;
  ...
  for (int i = 1; i < argc; ++i ) {
    ...
    if ( arg == "-s" || arg == "--save-frames" ) {
      ...
      removeArg( i, argc, argv );
    } else if ( (arg == "-B" || arg == "--benchmark-no-display") && !GLUTDisplayWrapper::m_requires_display ) {
      ...
      removeArg( i, argc, argv );
    } else if ( (arg == "-BB" || arg == "--timed-benchmark-no-display") && !GLUTDisplayWrapper::m_requires_display ) {
...
void GLUTDisplayWrapper::run( const std::string& title, SampleScene* scene, contDraw_E continuous_mode )
{
  if ( !m_initialized ) {
    std::cerr << "ERROR - GLUTDisplayTest::run() called before GLUTDisplayTest::init()" << std::endl;
    exit(2);
  }
  m_scene = scene;
  m_title = title;
...
void GLUTDisplayWrapper::setCamera(SampleScene::InitialCameraData& camera_data)
{
...
void GLUTDisplayWrapper::setCurContinuousMode(contDraw_E continuous_mode)
{
...
void GLUTDisplayWrapper::restartProgressiveTimer()
{
...
void GLUTDisplayWrapper::setContinuousMode(contDraw_E continuous_mode)
{
...
void GLUTDisplayWrapper::postRedisplay()
{
  glutPostRedisplay();
}

void GLUTDisplayWrapper::drawText( const std::string& text, float x, float y, void* font )
{
  // Save state
...
void GLUTDisplayWrapper::runBenchmarkNoDisplay( )
{
  // Set up scene
...
void GLUTDisplayWrapper::keyPressed(unsigned char key, int x, int y)
{
  try {
...
void GLUTDisplayWrapper::mouseButton(int button, int state, int x, int y)
{
  sutilCurrentTime( &m_start_time );
...
void GLUTDisplayWrapper::mouseMotion(int x, int y)
{
  sutilCurrentTime( &m_start_time );
...
void GLUTDisplayWrapper::resize(int width, int height)
{
  // disallow size 0
  width  = max(1, width);
  height = max(1, height);
...
void GLUTDisplayWrapper::idle()
{
  glutPostRedisplay();
}


void GLUTDisplayWrapper::displayFrame()
{
...
void GLUTDisplayWrapper::display()
{
...
void GLUTDisplayWrapper::quit(int return_code)
{
  try {
    if(m_scene)
    {
