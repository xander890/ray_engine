...
/*
 *  Implementation for the directional dipole for RGB colors.
 */
                             ...
                             const ScatteringMaterialProperties& properties)
{
  ...
  optix::float3 _r_tr_p1 = _r_tr + optix::make_float3(1.0f);
	...
	const MaterialDataCommon& material, unsigned int flags, TEASampler & sampler)
{
	...
	optix::float3 _r_sqr = optix::make_float3(dot(_x, _x));

	// distance to the real source
	optix::float3 _dr_sqr = _r_sqr;
	float dot_x_w12 = dot(_x, _w12);
	optix::float3 cos_beta = -sqrt(optix::make_float3(_r_sqr.x - dot_x_w12 * dot_x_w12) / (_r_sqr + properties.de*properties.de));
	...
	optix::float3 _dv_sqr = optix::make_float3(dot(_xoxv_r, _xoxv_r), dot(_xoxv_g, _xoxv_g), dot(_xoxv_b, _xoxv_b));

	// cosines of the virtual source
	optix::float3 _x_dot_wv = optix::make_float3(dot(_xoxv_r, _wv), dot(_xoxv_g, _wv), dot(_xoxv_b, _wv));
	optix::float3 _x_dot_no = optix::make_float3(dot(_xoxv_r, geometry.no), dot(_xoxv_g, geometry.no), dot(_xoxv_b, geometry.no));
	...
	return max(_Sd, optix::make_float3(0.0f)) * properties.global_coeff * (1 - R12) * F;
}
