...
void RayWorkbench::initialize_scene(GLFWwindow *)
{
    ...
    else
    {
        ...
        params.vfov = 53.0f;
        float ratio = parameters.screen_width / (float)parameters.screen_height;
    ...
    // Tone mapping pass
    rendering_output_buffer = create_glbo_buffer<optix::float4>(context, RT_BUFFER_INPUT_OUTPUT, RT_FORMAT_FLOAT4, parameters.screen_width, parameters.screen_height);
    context["output_buffer"]->setBuffer(rendering_output_buffer);

    tonemap_output_buffer = create_glbo_buffer<optix::uchar4>(context, RT_BUFFER_INPUT_OUTPUT, RT_FORMAT_UNSIGNED_BYTE4, parameters.screen_width, parameters.screen_height);
    context["tonemap_output_buffer"]->setBuffer(tonemap_output_buffer);

    debug_output_buffer = create_glbo_buffer<optix::uchar4>(context, RT_BUFFER_OUTPUT, RT_FORMAT_UNSIGNED_BYTE4, parameters.screen_width, parameters.screen_height);
    ...
    zoomed_area = optix::make_uint4(parameters.screen_width / 2 - 5, parameters.screen_height / 2 - 5, 10, 10);
    ...
    context["render_bounds"]->setUint(optix::make_uint4(0, 0, parameters.screen_width, parameters.screen_height));
...
void RayWorkbench::trace()
{
    ...
    unsigned int width = parameters.screen_width;
    unsigned int height = parameters.screen_height;

    t0 = currentTime();
    mScene->trace(width, height);
...
void RayWorkbench::do_resize(unsigned int w, unsigned int h)
{
    parameters.screen_width = w;
    parameters.screen_height = h;
    context["render_bounds"]->setUint(0, 0, w, h);
    resize_glbo_buffer<optix::float4>(rendering_output_buffer, w, h);
    resize_glbo_buffer<optix::char4>(tonemap_output_buffer, w, h);
    resize_glbo_buffer<optix::char4>(debug_output_buffer, w, h);
}

void RayWorkbench::resize(unsigned int width, unsigned int height)
{
    if (width != parameters.screen_width || height != parameters.screen_height)
...
{
    ar(
        cereal::make_nvp("screen_width", in.screen_width),
        cereal::make_nvp("screen_height", in.screen_height),
