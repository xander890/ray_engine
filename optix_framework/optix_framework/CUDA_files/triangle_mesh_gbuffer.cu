...
rtBuffer<float4, 1> normal_data;
rtBuffer<float4, 1> color_data;

RT_PROGRAM void mesh_intersect( int primIdx )
{
	if (ray.ray_type == dummy_ray_type)
	{
		float3 position = make_float3(vertex_data[launch_dim.x * launch_index.x + launch_index.y]);
		float intersection_distance = length(position - eye);
		//optix_print("Intersection: %f\n", intersection_distance);
		if (rtPotentialIntersection(intersection_distance))
		{
			texcoord = make_float3(0.0f);
			float3 normal = make_float3(normal_data[launch_dim.x * launch_index.x + launch_index.y]);
			geometric_normal = normal;
			shading_normal = normal;
			prd_cache.vertex = position;
			float3 color = make_float3(color_data[launch_dim.x * launch_index.x + launch_index.y]);
			//FIXME material & texcoord
			prd_cache.radiance.result = color;
...
//	if (ray.ray_type == dummy_ray_type)
//	{
//		aabb->m_min = make_float3(-MAX, -MAX, -MAX);
//		aabb->m_max = make_float3(MAX, MAX, MAX);
//		return;
//	}
//	else
	{
