...
_fn
void evaluate_no_light(const optix::float3& test, const optix::float3 & hit_normal, optix::float3& wi, optix::float3 & radiance, int & casts_shadows, TEASampler * sampler, unsigned int& light_index) { radiance = make_float3(1, 0, 0); }

_fn
int singular_light_size() { return singular_lights.size(); }

_fn
void evaluate_singular_light(const optix::float3 & hit_point, const optix::float3 & hit_normal, optix::float3& wi, optix::float3 & radiance, int & casts_shadows, TEASampler * sampler, unsigned int& light_index)
{
...
_fn void evaluate_area_light(const optix::float3& hit_point, const optix::float3& normal, optix::float3& wi, optix::float3 & radiance, int & casts_shadows, TEASampler * sampler, unsigned int& light_index, float tmin = scene_epsilon)
{
    //	assert(data != NULL);
    float zeta1 = sampler->next1D();
    float zeta2 = sampler->next1D();
    float zeta3 = sampler->next1D();
    int idx = optix::clamp((int)(area_lights.size() * zeta1), 0, area_lights.size() - 1);
    ...
    casts_shadows = 1;
    radiance = V * triangle.emission * fmaxf(dot(wi, normal), 0.0f) * fmaxf(dot(-wi, triangle.normal), 0.0f) / dist_sq * triangle.area * area_lights.size();
}

_fn void evaluate_direct_light(const optix::float3& hit_point, const optix::float3& normal, optix::float3& wi, optix::float3 & radiance, int & casts_shadows, TEASampler * sampler, unsigned int& light_index, float tmin = scene_epsilon)
{
...
_fn void sample_light(const optix::float3& position, const optix::float3 & normal, const unsigned int& ray_depth, TEASampler* sampler, optix::float3 & wi, optix::float3 & Li)
{
