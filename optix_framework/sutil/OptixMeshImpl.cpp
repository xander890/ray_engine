...
{
  throw Exception("Use beginLoad() and finalizeLoad() instead of loadModel()");
}

void OptixMeshImpl::beginLoad( const std::string& filename )
{
  if( m_currently_loading )
    throw Exception("Loading has already begun.");
...
{
  if( !m_currently_loading )
    throw Exception("finalizeLoad() can only be called after a call to beginLoad()");
...
struct OptixMeshImpl::ProcessGeometriesAfterLoadFunctor {
  ...
  void operator()( MeshGroup& group ) {
    ...
    if( m_mesh.m_large_geom ) {
      
      
      if( m_mesh.m_ASBuilder == std::string("Sbvh")
          || m_mesh.m_ASBuilder == std::string("KdTree"))
      {
        m_mesh.m_ASBuilder = "MedianBvh";
        m_mesh.m_ASTraverser = "Bvh";
      }
...
struct OptixMeshImpl::CreateGeometryInstancesFunctor
{
  ...
  void operator()( MeshGroup& group ) {
    ...
    GeometryInstance instance
      = m_mesh.m_context->createGeometryInstance( geo,
                                                  &material, &material + 1 );
...
void OptixMeshImpl::createGeoInstancesAndOptixGroup()
{
  ...
  m_geometrygroup->setChildCount( current_child_count 
                                  + static_cast<unsigned int>(instances.size()) );
  optix::Acceleration acceleration = m_context->createAcceleration(m_ASBuilder,
                                                                   m_ASTraverser);
