...
unsigned int   GLUTDisplay::m_timed_frames = 10000000u;
...
void GLUTDisplay::runBenchmarkNoDisplay( )
{
  ...
  // Initialize camera according to scene paramsm
...
void GLUTDisplay::display()
{
  ...
  if ( m_display_fps && m_cur_continuous_mode != CDNone && m_frame_count > 1 ) {
    ...
    if( dt > m_fps_update_threshold ) {
      sprintf( m_fps_text, "fps: %7.2f - %d", (m_frame_count - m_last_frame_count) / dt, m_frame_count );
  ...
  std::string debug;
  float x, y;
  m_scene->getDebugText(debug, x, y);
  drawText(debug, x, y, GLUT_BITMAP_8_BY_13);
